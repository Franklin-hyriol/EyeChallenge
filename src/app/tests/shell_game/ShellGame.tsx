...
export default function ShellGame() {
  ...
  const getFeedback = (finalScore: number) => {
    if (finalScore === ROUNDS)
      return {
        title: "Perfect!",
        message: "Your tracking skills are flawless!",
        icon: FiAward,
        className: "alert-success",
      };
    if (finalScore >= ROUNDS * 0.7)
      return {
        title: "Excellent!",
        message: "You have sharp visual tracking.",
        icon: FiCheckCircle,
        className: "alert-info",
      };
    return {
      title: "Good Effort!",
      message: "Practice makes perfect. Keep it up!",
      icon: FiEye,
  ...
  const getStatusMessage = () => {
    switch (status) {
      case "highlighting":
        return "Memorize the green circle!";
      case "shuffling":
  ...
  const isGameOver =
    (status === "result" && round >= ROUNDS) ||
    (status === "idle" && round >= ROUNDS);
  if (isGameOver) {
    ...
    <div ref={gameAreaRef} className="w-full flex flex-col items-center pt-10">
      ...
      <div className="w-full mt-10 md:mt-12 max-w-4xl">
        ...
        <div
          role="alert"
          className={clsx(
            "alert alert-info flex justify-center mt-4 md:mt-6 w-fit m-auto"
          )}
        >
          <FiEye />
            ...
            <div
              ...
              className={clsx(
                "absolute w-20 h-20 rounded-full transition-colors",
                {
                  ...
                  "bg-green-500":
                    (status === "highlighting" && circle.isTarget) ||
                    (status === "feedback" && circle.isTarget),
                  "bg-red-500":
                    status === "feedback" &&
                  ...
                  // Default color
                  "bg-primary":
                    status !== "highlighting" && status !== "feedback",
              ...
              style={{
                transition: `transform ${
                  shuffleSpeed / 3
                }ms ease-in-out, background-color 300ms`,
