...
export default function VisualMemoryGame() {
    ...
    const getStatusMessage = () => {
      if (status === 'memorize') return `Memorize ${targetShapes.length} shape(s)!`;
      if (status === 'recall') return `Select the ${targetShapes.length} shape(s) you saw.`;
      return '';
    };
    ...
    if (status === 'idle') {
      return (
        <div className="flex-1 flex flex-col items-center justify-center my-10 md:my-12">
          <button className="btn btn-lg btn-primary" onClick={startGame}>Start Test</button>
        </div>
      );
    }
      ...
      return (
        <>
          <div className="text-center my-10 md:my-12 max-w-md mx-auto">
            ...
            <div role="alert" className={clsx('alert', finalScore > 5 ? 'alert-success' : 'alert-info')}>
              {finalScore > 5 ? <FiAward /> : <FiEye />}
              <span>{finalScore > 5 ? 'Excellent memory!' : 'Good job! Keep practicing.'}</span>
            </div>
          </div>
          <div className="flex items-center gap-4 mt-8 justify-center flex-wrap">
            <button className="btn btn-lg btn-outline" onClick={handleShare}><FiShare2 /> Share</button>
            <button className="btn btn-lg btn-primary" onClick={startGame}><TbReload /> Play Again</button>
            <button className="btn btn-lg btn-secondary" onClick={() => router.push(`/${nextTest.link}`)}>Next Challenge <FiNext /></button>
          </div>
        </>
      );
    }
        ...
        <div className="w-full flex items-center justify-center my-10 md:my-12 min-h-[300px]">
          {status === 'memorize' && targetShapes.length > 0 && (
            <div className="flex flex-wrap justify-center items-center gap-4 animate-in fade-in">
              {targetShapes.map((shape: ShapeType) => <Shape key={shape} type={shape} className="text-primary" />)}
            </div>
          )}
          {status === 'recall' && (
            <div className={clsx("grid gap-4 animate-in fade-in", {
                'grid-cols-2': gridShapes.length === 4,
                'grid-cols-3': gridShapes.length === 6 || gridShapes.length === 9,
            })}>
                ...
                <button
                  ...
                  className={clsx(
                    "p-4 rounded-lg border-2 transition-all",
                    {
                      ...
                      'border-green-500 bg-green-500/20': feedback === 'correct' && targetShapes.includes(shape),
                      'border-red-500 bg-red-500/20': feedback === 'incorrect' && userSelection.includes(shape) && !targetShapes.includes(shape),
                    }
                  )}
                >
